...
#          diag=FALSE 
# )
# cairo_ps(file = "corr_frcnn.eps", onefile = TRUE, fallback_resolution = 600, width=9, height=9)
# library(RColorBrewer)
# par(mar=c(0,0,0,0))
# corrplot(data_frcnn[1:96,1:96], type="upper", order="hclust", is.corr = FALSE,
#         tl.col="black", tl.srt=45, tl.cex = 0.4, #Text label color and rotation
#         p.mat = 1- data_frcnn[1:96,1:96], sig.level = 0.99, insig = "blank", mar = c(0, 0, 0, 0),
#          col=brewer.pal(n=8, name="RdBu"))
# dev.off()

cairo_ps(file = "corr_kpn.eps", onefile = TRUE, fallback_resolution = 600, width=9, height=9)
library(RColorBrewer)
par(mar=c(0,0,0,0))
corrplot(data_kgpnet[1:96,1:96], type="upper", order="hclust", is.corr = FALSE,
         tl.col="black", tl.srt=45, tl.cex = 0.4, #Text label color and rotation
